...
abstract class elFinderVolumeDriver {
	...
	public function mount(array $opts) {
		...
		$chk1 = @finfo_file(finfo_open(FILEINFO_MIME), __FILE__);
		$chk2 = explode(';', $chk1);
		$chk3 = array_shift($chk2);

		if (($type == 'finfo' || $type == 'auto')
			&& class_exists('finfo')
			&& preg_match($regexp, $chk3)) {
				$type = 'finfo';
	...
	public function paste($volume, $src, $dst, $rmSrc = false) {
		...
		if ($volume == $this) {
			...
			if ($this->_inpath($destination, $source)) {
				return $this->setError(elFinder::ERROR_COPY_INTO_ITSELF, $destination);
			}
	...
	protected function attr($path, $name, $val=false) {
		...
		if ($this->access) {
			if (is_array($this->access)) {
				...
				//$func = $this->access;
				$perm = $this->access($name, $path, $this->options['accessControlData'], $this);
			}
